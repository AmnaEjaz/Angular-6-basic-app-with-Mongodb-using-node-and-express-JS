...
// Defined description route
adUnitRoutes.route('/view/:id').get(function (req, res) {
  let id = req.params.id;
  AdUnit.findById(id, function (err, adUnit){
      res.json(adUnit);
  });
...
adUnitRoutes.route('/update/:id').post(function (req, res) {
    ...
    if (!adUnit)
      return next(new Error('Could not load Document'));
        ...
        adUnit.price = req.body.price;
        adUnit.description = req.body.description;
        adUnit.image = req.body.image;
        adUnit.currency = req.body.currency;
        adUnit.link = req.body.link;
